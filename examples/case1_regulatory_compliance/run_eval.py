...
async def run_tc101(cli: CLIRunner, report: AcptReport):
    t0 = time.time()
    try:
        r1 = await cli.remember("Supplier SUP_A: 深圳智造科技有限公司, registered_capital 50M CNY", tags={"tag": "supplier", "tag_2": "SUP_A"}, created_by="compliance_test")
        r2 = await cli.remember("Supplier SUP_B: 某贸易有限公司, registered_capital 1M CNY", tags={"tag": "supplier", "tag_2": "SUP_B"}, created_by="compliance_test")
        r3 = await cli.remember("CoreEnterprise CE_HW: 华为技术有限公司, credit_rating AAA, is_in_regulatory_whitelist=True", tags={"tag": "core_enterprise", "tag_2": "CE_HW"}, created_by="compliance_test")
        r4 = await cli.remember("Invoice INV_001: amount 5M, supplier SUP_A, verified=True", tags={"tag": "invoice", "tag_2": "evidence"}, created_by="compliance_test")
...
async def run_tc102(cli: CLIRunner, report: AcptReport):
    ...
    try:
        for i, (sid, name, capital) in enumerate([("SUP_A", "深圳智造科技", "50M"), ("SUP_B", "某贸易公司", "1M"), ("SUP_C", "新兴科技", "5M"), ("SUP_D", "卓越制造", "100M"), ("SUP_E", "部分担保供应商", "20M")]):
            await cli.remember(f"Supplier {sid}: {name}, registered_capital {capital} CNY, status ACTIVE", tags={"tag": "supplier", "tag_2": sid, "tag_3": "baseline"}, created_by="compliance_test")
        await cli.remember("CoreEnterprise CE_HW: 华为, is_in_regulatory_whitelist=True", tags={"tag": "core_enterprise", "tag_2": "whitelist"}, created_by="compliance_test")
...
async def run_tc103(cli: CLIRunner, report: AcptReport):
    t0 = time.time()
    try:
        await cli.remember("Whitelist v2026.04.2 update: added SUP_C5 as pilot enterprise", tags={"tag": "whitelist", "tag_2": "v2026.04.2", "tag_3": "update"}, memory_type="observation", created_by="compliance_test")
        await cli.remember("Supplier SUP_C5: 试点企业, registered_capital 30M CNY, is_in_regulatory_whitelist=True", tags={"tag": "supplier", "tag_2": "SUP_C5", "tag_3": "whitelist"}, created_by="compliance_test")
...
async def run_tc104(cli: CLIRunner, report: AcptReport):
    ...
    try:
        ...
        await cli.remember("Whitelist v2026.04.2: SUP_C5 is approved pilot enterprise", tags={"tag": "whitelist", "tag_2": "v2026.04.2"}, memory_type="observation", created_by="compliance_test")
        ...
        await cli.remember("Version rollback: whitelist reverted from v2026.04.2 to v2026.04.1. SUP_C5 approval revoked.", tags={"tag": "whitelist", "tag_2": "rollback", "tag_3": "v2026.04.1"}, memory_type="observation", created_by="compliance_test")
...
async def run_tc105(cli: CLIRunner, report: AcptReport):
    t0 = time.time()
    try:
        await cli.remember("银发〔2026〕23号 第八条: 核心企业白名单制度，供应商需通过核心企业担保", tags={"tag": "regulation", "tag_2": "circular23", "tag_3": "article8"}, memory_type="observation", created_by="compliance_test")
        await cli.remember("Invoice evidence: SUP_A invoice INV_001 verified, amount 5M CNY", tags={"tag": "evidence", "tag_2": "invoice", "tag_3": "SUP_A"}, created_by="compliance_test")
...
async def run_tc106(cli: CLIRunner, report: AcptReport):
    t0 = time.time()
    try:
        await cli.remember("Compliance risk report: 5 suppliers analyzed, 1 passed (20%), 4 failed. Key finding: whitelist coverage low. Risk level: HIGH. Action: expand whitelist or reject non-compliant suppliers.", tags={"tag": "report", "tag_2": "compliance", "tag_3": "risk"}, memory_type="observation", created_by="compliance_test")
