...
async def run_t1_rule_contradiction(cli: CLIRunner, report: EvalReport):
    t0 = time.time()
    try:
        await cli.remember("华信科技风险等级D级，存在高违约风险", memory_type="opinion", tags=["risk", "contradiction_test"])
        await cli.remember("华信科技风险等级不是D级，已调整为C级", memory_type="opinion", tags=["risk", "contradiction_test"])
...
async def run_t2_llm_contradiction(cli: CLIRunner, report: EvalReport):
    t0 = time.time()
    try:
        await cli.remember("中芯科技财务稳健，资产负债率仅35%", memory_type="observation", tags=["semiconductor", "financial"])
        await cli.remember("中芯科技负债率高达65%，现金流紧张", memory_type="observation", tags=["semiconductor", "financial"])
...
async def run_t4_correction_propagation(cli: CLIRunner, report: EvalReport):
    t0 = time.time()
    try:
        r1 = await cli.remember("瑞芯微电风险等级B级，营收稳定增长", memory_type="entity", tags=["semiconductor"])
        entity_id = r1.get("node_id")
        await cli.remember("瑞芯微电2025年Q4营收同比增长20%", memory_type="observation", tags=["semiconductor"])
...
async def run_t6_reflect_search_sequence(cli: CLIRunner, report: EvalReport):
    ...
    try:
        ...
        await cli.remember("华信科技：风险等级C级，重组中", memory_type="entity", tags=["company"])
